...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      const result = await pool.query(
        'SELECT id, email, password_hash, created_at FROM customers WHERE email = $1',
        [email]
      );
...
router.get('/me', authenticateCustomerToken, async (req, res) => {
  try {
    const result = await pool.query('SELECT id, email, created_at FROM customers WHERE id = $1', [req.customerId]);
